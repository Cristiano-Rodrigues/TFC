...
Face a estas limitações, a arquitectura aqui desenvolvida utiliza LLMs não como fonte autónoma de conhecimento, mas como componente de geração de respostas dentro de um pipeline RAG, onde o contexto relevante é previamente recuperado da base documental da organização. Esta abordagem mitiga as limitações referidas, ancorando as respostas do modelo em documentos concretos e verificáveis.

### 2.2.4. \textit{Embeddings} e Representação Vectorial de Texto

Os \textit{embeddings} constituem representações numéricas de texto em espaços vectoriais de alta dimensionalidade, nas quais a proximidade geométrica entre vectores reflecte a similaridade semântica entre os textos correspondentes. Esta técnica é fundamental para o funcionamento da busca semântica no sistema proposto.
...
@thakurBEIRHeterogenousBenchmark2021 propõem o BEIR (*Benchmarking Information Retrieval*), um benchmark heterogéneo para a avaliação de modelos de recuperação de informação em cenários de transferência zero-shot. Os autores demonstram que, embora os métodos de recuperação densa apresentem resultados promissores em domínios específicos, a sua capacidade de generalização para domínios não vistos durante o treino permanece limitada em comparação com métodos léxicos como o BM25. Esta constatação sugere que abordagens híbridas, que combinam busca semântica e léxica, podem oferecer uma robustez superior em cenários práticos. Contudo, é fundamental notar que o modelo seleccionado para o presente trabalho --- o *embed-multilingual-v3.0* da Cohere --- apresenta um desempenho notável em cenários *zero-shot* e uma eficácia de topo em diversas línguas quando submetido a avaliações rigorosas semelhantes ao BEIR [@cohereMultilingual2023]. Esta elevada capacidade de generalização e compreensão multilíngue mitiga parcialmente a necessidade imediata de uma abordagem híbrida, justificando a sua adopção como motor exclusivo de busca semântica no protótipo desenvolvido, embora a integração de busca léxica permaneça como uma via de desenvolvimento futuro.
...
No contexto deste trabalho, adopta-se a concepção fraca de agente proposta por @wooldridgeIntroductionMultiagentSystems2012, considerando-se agente um componente de software especializado, orientado para um objectivo específico, capaz de perceber informação do seu ambiente, tomar decisões relativamente à tarefa que lhe foi atribuída e executar acções através de ferramentas externas. Esta definição não pressupõe autonomia cognitiva forte, mas requer que o componente possua autonomia operacional. Com base nesta definição, o sistema proposto estabelece uma separação estrutural clara: o **agente** é a entidade inteligente baseada em LLM que raciocina e toma decisões; o ***workflow*** é estritamente um mecanismo sequencial de execução e integração; e o **n8n** actua exclusivamente como a plataforma de orquestração. Fica assim patente que a autonomia operacional e cognitiva reside nos agentes, enquanto a coordenação dos eventos e do encaminhamento de dados permanece centralizada no orquestrador.
...
O advento dos Modelos de Linguagem de Grande Escala trouxe uma transformação profunda ao campo dos Sistemas Multiagentes, permitindo a criação de agentes com capacidades linguísticas e de raciocínio sem precedentes. @wangSurveyLargeLanguage2023 apresentam uma revisão abrangente dos agentes autónomos baseados em LLMs, propondo um enquadramento unificado que identifica quatro componentes essenciais: o módulo de perfil (que define o papel e os objectivos do agente), o módulo de memória (que armazena experiências e contexto), o módulo de planeamento (que decompõe tarefas complexas em subtarefas e permite auto-reflexão) e o módulo de acção (que executa as tarefas determinadas pelo planeamento). Os autores analisam as aplicações destes agentes em domínios como as ciências sociais, as ciências naturais e a engenharia.

@schickToolformerLanguageModels2023 demonstraram que os LLMs podem aprender, de forma auto-supervisionada, a utilizar ferramentas externas através de APIs, decidindo quando invocar uma ferramenta, quais argumentos passar e como integrar os resultados na geração subsequente. É precisamente esta capacidade de utilização de ferramentas (*tool use*) que permite que agentes baseados em LLMs consultem dinamicamente bases de dados, interajam com APIs externas e executem acções sobre o sistema. Ao transcenderem a mera geração de texto, estes agentes tornam possível a automação inteligente da gestão da informação, agindo como entidades operacionais autónomas no tratamento do conhecimento.
...
A escolha do pgvector sobre bases de dados vectoriais nativas justifica-se por várias razões no contexto deste trabalho. A razão principal reside na possibilidade de unificar os dados relacionais (utilizadores, documentos, permissões) e os dados vectoriais (\textit{embeddings}) numa única plataforma, simplificando a arquitectura e eliminando a necessidade de sincronização entre sistemas. Além disso, esta abordagem possibilita a implementação de consultas híbridas que combinam filtros relacionais (departamento, cargo, permissões) com buscas por similaridade vectorial, através da função `match_chunks()` implementada em PL/pgSQL. Por último, o PostgreSQL é suportado nativamente pela plataforma Supabase, que fornece adicionalmente serviços de autenticação, armazenamento de ficheiros e APIs REST automáticas, reduzindo significativamente a complexidade da infra-estrutura.
...
No sistema proposto, o n8n actua estritamente como a plataforma de orquestração central. A sua função é coordenar *workflows* e eventos, integrar serviços externos e encaminhar dados entre os agentes especializados (baseados em LLMs). O n8n não é, por si só, um agente inteligente, e os seus *workflows* são meros mecanismos de execução e não entidades cognitivas. Através do n8n, os eventos de ingestão e consulta são capturados e reencaminhados para os agentes competentes. Por exemplo, na ingestão, o n8n orquestra o fluxo que entrega os documentos aos agentes para que estes efectuem a análise, estruturação e solicitação de geração de *embeddings*; na consulta, o n8n recebe o pedido, invoca as funções de recuperação na base de dados (como a função `match_chunks()`) e fornece o contexto ao agente de resposta. Esta arquitectura modular assegura que a responsabilidade de encaminhamento e integração permanece no orquestrador, enquanto o processamento analítico e as decisões baseadas na linguagem natural residem inteiramente nos agentes de IA.
...
Adicionalmente, na adopção de Modelos de Linguagem e metodologias RAG, a auditoria de conformidade com a legislação de proteção de dados adquire especial relevância. A submissão de fragmentos de texto corporativo a APIs externas impõe riscos de partilha indevida de Dados Pessoais Sensíveis (PII). Assim, é crucial incorporar na arquitectura mecanismos que ajustem as instruções (*prompts*) do sistema para que este inspeccione e reveja o texto de saída gerado, com o objectivo de mitigar, no texto da resposta, a exposição de dados pessoais sensíveis.
...
**Plataformas empresariais tradicionais.** O Microsoft SharePoint [@microsoftSharePoint2024] e o Atlassian Confluence [@atlassianConfluence2024] constituem as soluções mais amplamente adoptadas para gestão documental em contexto empresarial. Estas plataformas oferecem funcionalidades robustas de armazenamento, organização e colaboração documental, incluindo controlo de versões, fluxos de aprovação e integração com outros produtos dos respectivos ecossistemas. No entanto, as suas capacidades de recuperação de informação baseiam-se predominantemente em busca por palavras-chave, sem mecanismos nativos de compreensão semântica. A incorporação recente de funcionalidades de IA (Microsoft Copilot no SharePoint, Atlassian Intelligence no Confluence) representa uma evolução significativa, embora estas funcionalidades estejam frequentemente limitadas aos planos empresariais de custo elevado e dependam de infra-estruturas proprietárias.

**Plataformas com IA nativa.** Uma nova geração de ferramentas integra capacidades de IA como componente central da experiência. O Notion AI [@notionAI2024] combina gestão de documentos e bases de conhecimento com funcionalidades de geração e sumarização de texto baseadas em LLMs. O Glean [@gleanAI2024] oferece busca empresarial unificada com compreensão semântica, indexando conteúdo de múltiplas fontes (email, documentos, mensagens). O Guru [@guruAI2024] disponibiliza uma base de conhecimento com verificação automática de actualidade e sugestões contextuais. Estas soluções demonstram o potencial da IA aplicada à gestão da informação, mas apresentam limitações comuns: são predominantemente proprietárias, operam em infra-estruturas centralizadas fora do controlo da organização, têm custos de subscrição elevados e oferecem capacidades limitadas de personalização das políticas de acesso.

**Projectos open-source.** O Onyx (anteriormente designado Danswer) [@danswerOnyx2024] constitui a referência mais próxima ao sistema proposto no espaço open-source. Trata-se de uma plataforma de perguntas e respostas empresarial que utiliza RAG para recuperar informação de múltiplas fontes (documentos, Slack, email) e gerar respostas contextualizadas. Embora partilhe princípios arquitecturais semelhantes ao sistema proposto, o Onyx não foi concebido para contextos multi-tenant com isolamento por empresa, não oferece RBAC refinado por departamento, e a sua arquitectura não foi optimizada para o contexto linguístico e organizacional angolano.
...
\hline
Código aberto & Não & Não & Não & Não & Sim & \textbf{Parcial (fair-code)} \\
\hline
