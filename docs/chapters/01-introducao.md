...
O presente trabalho circunscreve-se ao desenvolvimento de um protótipo de prova de conceito, não constituindo uma solução de produção pronta para implantação em larga escala. A validação do sistema é realizada num ambiente controlado de simulação empresarial, composto por dois *tenants* de teste (Empresa A e Empresa B) com utilizadores fictícios alocados a diferentes departamentos e níveis hierárquicos, conforme descrito na secção 3.3. Na sua fase actual, o sistema processa exclusivamente informação em formato de texto — documentos PDF, ficheiros de texto e páginas Wiki — não abrangendo conteúdos multimédia como imagem, áudio ou vídeo, nem fontes externas como email ou canais de mensagens, embora a arquitectura modular do n8n permita a integração futura de novos conectores para essas fontes. Finalmente, o âmbito da investigação está circunscrito ao contexto organizacional angolano, pelo que a generalização dos resultados a outros contextos requer estudos complementares.
