...
3.  **Modelagem e Implementação:** A modelagem baseada em UML e o posterior desenvolvimento do protótipo demonstraram que o modelo de **Abstração Unificada** — agregando PDFs, ficheiros e artigos de Wiki sob a entidade única `documents` diferenciados por `source_type` — é eficiente. Esta escolha simplificou o pipeline de fragmentação e vectorização e facilitou a governança de dados.
