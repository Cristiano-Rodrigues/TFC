...
Este estudo propôs-se a avaliar a viabilidade técnica de um protótipo de sistema de gestão da informação organizacional baseado em Inteligência Artificial, capaz de centralizar e recuperar dados em ambientes empresariais angolanos. Através da implementação de uma arquitectura *Retrieval-Augmented Generation* (RAG) suportada por uma interface Next.js, uma base de dados relacional vectorial (PostgreSQL com `pgvector`) e orquestração visual via n8n, logrou-se construir uma solução eficiente baseada no modelo de Abstração Unificada documental.

Os testes realizados, nos cenários avaliados, não permitiram o acesso a documentos de outro tenant nem a documentos fora do perímetro de autorização do utilizador, fornecendo evidências preliminares da eficácia das políticas implementadas para o isolamento de inquilinos (*multi-tenancy*) e segurança departamental (RBAC). Com tempos médios de pesquisa inferiores a 10 segundos e respostas factualmente alinhadas ao contexto autorizado, conclui-se que a adoção de sistemas de inteligência documental através de uma arquitectura predominantemente baseada em componentes de código-fonte disponível e auto-hospedáveis, complementada por serviços de IA externos, é viável e operacional, proporcionando um caminho concreto para a aceleração da maturidade digital nas organizações em Angola.
